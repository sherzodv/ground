...
pub fn render(req: &RenderReq, ctx: &Value) -> Result<Vec<JsonUnit>, GenError> {
    ...
    tera.add_raw_templates(req.units.iter().map(|u| (u.file.as_str(), u.template.as_str())).collect::<Vec<_>>())
    ...
    let files = manifest_value.get("files")
        ...
        .ok_or_else(|| GenError::Manifest { cause: "missing 'files' array".into() })?;

    files.iter()
        .map(|file| {
            let path = file.get("file")
                .and_then(Value::as_str)
                .ok_or_else(|| GenError::Manifest { cause: "file entry missing string 'file'".into() })?;
