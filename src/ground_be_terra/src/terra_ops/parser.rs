...
pub enum TfEvent {
    ...
    RefreshStart     { address: String },
    RefreshComplete  { address: String },
    PlannedChangesStart,
...
pub struct TfParser { pub mode: Mode }
...
fn parse_json(line: &str) -> Option<TfEvent> {
    ...
    match v["type"].as_str()? {
        ...
        "planned_change" => {
            ...
            Some(TfEvent::ResourcePlanned { address: addr, action })
        }
        "apply_start" => {
            let addr   = v["hook"]["resource"]["addr"].as_str()?.to_string();
            let action = parse_action(v["hook"]["action"].as_str().unwrap_or(""))?;
            Some(TfEvent::ResourceApplying { address: addr, action })
        }
        "apply_complete" => {
            let addr    = v["hook"]["resource"]["addr"].as_str()?.to_string();
            let action  = parse_action(v["hook"]["action"].as_str().unwrap_or(""))?;
            let elapsed = v["hook"]["elapsed_seconds"].as_f64().unwrap_or(0.0) as u32;
            Some(TfEvent::ResourceDone { address: addr, action, elapsed_secs: elapsed })
        }
        "apply_errored" => {
            let addr = v["hook"]["resource"]["addr"].as_str()?.to_string();
        ...
        "resource_drift" => {
            let addr   = v["change"]["resource"]["addr"].as_str()?.to_string();
            let action = parse_action(v["change"]["action"].as_str().unwrap_or(""))?;
            Some(TfEvent::ResourceDrift { address: addr, action })
        }
        "diagnostic" => {
            let severity = v["diagnostic"]["severity"].as_str().unwrap_or("error").to_string();
            let summary  = v["diagnostic"]["summary"].as_str().unwrap_or("").to_string();
            let detail   = v["diagnostic"]["detail"].as_str()
                .filter(|s| !s.is_empty())
                .map(str::to_string);
            let address  = v["diagnostic"]["address"].as_str()
                .map(str::to_string);
            Some(TfEvent::Diagnostic { severity, summary, detail, address })
        }
        _ => None,
    }
}
