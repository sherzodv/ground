...
mod golden {
    ...
    use ground_compile::{compile, CompileReq, Unit, CompileRes, AsmDef, AsmField, asm_value_to_json};
    ...
    fn load_tera_dir(dir: &Path) -> Result<Tera, String> {
        ...
        tera.add_raw_templates(units.iter().map(|(name, src)| (name.as_str(), src.as_str())).collect::<Vec<_>>())
    ...
    fn render_dir(dir: &Path, ctx: &Value) -> Result<String, String> {
        ...
        tera.render("manifest.json.tera", &tera_ctx).map_err(|e| e.to_string())
    }
    ...
    fn run_fixture(dir: &Path) -> Result<(), String> {
        ...
        let units = load_units(dir)
            .map_err(|e| format!("{}: load units error: {e}", dir.display()))?;
        let rendered = render_units(&RenderReq {
        ...
        let mut actual_files: Vec<(String, String)> = rendered.into_iter()
            .map(|u| (u.file, u.content))
            .collect();
        actual_files.sort_by(|a, b| a.0.cmp(&b.0));

        let expected_names: Vec<String> = expected_files.iter().map(|(name, _content): &(String, String)| name.clone()).collect();
        let actual_names: Vec<String> = actual_files.iter().map(|(name, _content): &(String, String)| name.clone()).collect();
        if actual_names != expected_names {
            return Err(format!(
                "{}: rendered files mismatch\n--- expected ---\n{:?}\n--- actual ---\n{:?}",
                dir.display(), expected_names, actual_names,
            ));
        }
        ...
        for ((name, actual), (_, expected)) in actual_files.iter().zip(expected_files.iter()) {
            if normalize_trailing_newlines(actual) != normalize_trailing_newlines(expected) {
                return Err(format!(
                    ...
                    dir.display(), name, expected, actual,
                ));
            }
        }
    ...
    fn collect_expected_files(root: &Path, dir: &Path, out: &mut Vec<(String, String)>) -> Result<(), String> {
