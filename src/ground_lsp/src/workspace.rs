...
impl Workspace {
    pub fn new(root: PathBuf) -> Self {
        Self { root, open_docs: HashMap::new(), analysis: None }
    }

    pub fn did_open(&mut self, params: &Value) {
        let Some(uri) = params.get("textDocument").and_then(|v| v.get("uri")).and_then(Value::as_str) else { return; };
        let Some(text) = params.get("textDocument").and_then(|v| v.get("text")).and_then(Value::as_str) else { return; };
        self.open_docs.insert(uri.to_string(), text.to_string());
    }

    pub fn did_change(&mut self, params: &Value) {
        let Some(uri) = params.get("textDocument").and_then(|v| v.get("uri")).and_then(Value::as_str) else { return; };
        let Some(text) = params.get("contentChanges").and_then(Value::as_array).and_then(|v| v.last()).and_then(|v| v.get("text")).and_then(Value::as_str) else { return; };
        self.open_docs.insert(uri.to_string(), text.to_string());
    }
    ...
    pub fn reload(&mut self) -> io::Result<()> {
        let files = collect_user_files(&self.root, &self.open_docs)?;
        let units: Vec<Unit> = files.iter().map(|f| Unit {
            name: f.name.clone(),
        ...
        self.analysis = Some(WorkspaceAnalysis { files, res, ts_scope_to_uri, ts_functions_by_uri });
        Ok(())
    }

    pub fn diagnostics_by_uri(&self) -> BTreeMap<String, Vec<Value>> {
        let mut out: BTreeMap<String, Vec<Value>> = BTreeMap::new();
        let Some(analysis) = &self.analysis else { return out; };
        ...
        for err in &analysis.res.errors {
            let Some(uri) = error_uri(analysis, err.loc.as_ref()) else { continue; };
    ...
    pub fn analysis_scope_and_unit_for_uri(&self, uri: &str) -> Option<(&WorkspaceAnalysis, ScopeId, u32)> {
        let analysis = self.analysis.as_ref()?;
        for (i, file) in analysis.files.iter().enumerate() {
            if file.uri == uri || file.ts_uri == uri {
                let unit = i as u32;
...
pub fn resolve_ground_token(analysis: &WorkspaceAnalysis, scope: ScopeId, token: &str) -> Option<Value> {
...
pub fn resolve_mapper_token(analysis: &WorkspaceAnalysis, scope: ScopeId, token: &str) -> Option<Value> {
    let ir = &analysis.res.ir;
...
pub fn describe_ground_token<'a>(analysis: &'a WorkspaceAnalysis, scope: ScopeId, token: &str) -> Option<(&'static str, String)> {
    let ir = &analysis.res.ir;
    if let Some(def_id) = lookup_def_visible(ir, scope, token) {
...
pub fn definition_from_ts(analysis: &WorkspaceAnalysis, uri: &str, pos: Position) -> Option<Vec<Value>> {
    ...
    let defs: Vec<Value> = analysis.res.ir.defs.iter()
...
pub fn lookup_pack_visible(ir: &IrRes, mut scope: ScopeId, name: &str) -> Option<ScopeId> {
    loop {
        ...
        let Some(parent) = sc.parent else { return None; };
        scope = parent;
    }
}
...
pub fn lookup_shape_visible(ir: &IrRes, mut scope: ScopeId, name: &str) -> Option<ground_compile::ir::ShapeId> {
    loop {
        let sc = ir.scopes.get(scope.0 as usize)?;
...
fn lookup_def_in_scope(ir: &IrRes, scope: ScopeId, name: &str) -> Option<DefId> {
    ir.scopes.get(scope.0 as usize)?.defs.get(name).and_then(|v| v.first()).copied()
}

fn lookup_shape_in_scope(ir: &IrRes, scope: ScopeId, name: &str) -> Option<ground_compile::ir::ShapeId> {
...
pub fn scan_ts_functions(src: &str) -> Vec<TsFn> {
    ...
    for needle in ["export function ", "function "] {
        while let Some(idx) = src[search_from..].find(needle) {
            ...
            out.push(TsFn { name, start, end: name_start + name_len });
...
fn collect_user_files(root: &Path, overrides: &HashMap<String, String>) -> io::Result<Vec<UserFile>> {
    let mut files = vec![];
...
fn collect_user_files_recursive(
    ...
    for entry in fs::read_dir(dir)? {
        ...
        let name = if stem == "pack" { String::new() } else { stem.to_string() };
        let abs_path = fs::canonicalize(&path).unwrap_or(path.clone());
        let uri = file_uri(&abs_path)?;
        let grd_src = overrides.get(&uri).cloned().unwrap_or(fs::read_to_string(&path)?);
        ...
        let ts_src = if ts_path.exists() {
            Some(overrides.get(&ts_uri).cloned().unwrap_or(fs::read_to_string(&ts_path)?))
        } else {
            overrides.get(&ts_uri).cloned()
        };
        files.push(UserFile { uri, pack_path, name, grd_src, ts_uri, ts_src });
    }
    Ok(())
}
