...
pub fn lower(ir: &IrRes, ts_src: &str) -> AsmRes {
    ...
    let defs: Vec<AsmDef> = ir.defs.iter().enumerate()
        ...
        resolve_named_def(root_id, ir, ts_src, &mut cache, &mut resolving, &mut resolved_order)
    }).collect();
...
fn resolve_named_def(
    ...
    if !resolving.insert(id) {
        ...
        return AsmDef {
            type_name: ir.shapes[def.shape_id.0 as usize].name.clone().unwrap_or_else(|| "_".into()),
            name: def.name.clone(),
...
fn resolve_def_with_overrides(
    ...
    let def = &ir.defs[id.0 as usize];
    let type_name = ir.shapes[def.shape_id.0 as usize].name.clone().unwrap_or_else(|| "_".into());
    let name = def.name.clone();
    let type_hint = def.type_hint.clone();

    let effective_fields = merge_ir_fields(&def.fields, overrides);
    ensure_named_deps_resolved(&effective_fields, ir, ts_src, cache, resolving, resolved_order);

    let base_def = def.base_def.map(|base_id| {
        resolve_named_or_inline(base_id, &effective_fields, ir, ts_src, cache, resolving, resolved_order)
    });

    let mut fields = base_def.as_ref().map(|def| def.fields.clone()).unwrap_or_default();
    ...
    if let Some(mapper_fn) = &def.mapper_fn {
        ...
        if has_ts_fn(ts_src, &ascent_fn) {
            ...
            let input_json = build_input_json(&effective_fields, &def.inputs, ir, cache);
            apply_mapper_output(&mut fields, ts_src, &ascent_fn, &[resolved_json, input_json]);
        }
    }

    AsmDef { type_name, name, type_hint, fields }
}
    ...
    match value {
        IrValue::Inst(fid) => {
            ...
            if def.name != "_" {
                ...
                ensure_named_deps_resolved(&def.fields, ir, ts_src, cache, resolving, resolved_order);
            }
        }
        IrValue::List(items) => {
            for item in items {
...
fn apply_mapper_output(fields: &mut Vec<AsmField>, ts_src: &str, fn_name: &str, args: &[String]) {
    ...
    if let Ok(output_json) = output {
        if let Ok(serde_json::Value::Object(map)) = serde_json::from_str::<serde_json::Value>(&output_json) {
            for (k, v) in map {
                merge_asm_field(fields, AsmField { name: k, value: json_val_to_asm_value(&v) });
            }
        }
    }
}
            ...
            let (value, asm_payload) = match &td.body {
                IrShapeBody::Enum(vs) => match vs[*idx as usize].segments.first().map(|s| &s.value) {
                    ...
                    Some(IrRefSegValue::Shape(vtid)) => {
                        let variant_type_name = ir.shapes[vtid.0 as usize].name.clone().unwrap_or_else(|| "_".into());
        ...
        IrValue::Inst(fid) => {
            ...
            if mapping.name == "_" {
                ...
                AsmValue::Def(Box::new(AsmDef {
                    type_name: ir.shapes[mapping.shape_id.0 as usize].name.clone().unwrap_or_else(|| "_".into()),
                    name: "_".into(),
                    type_hint,
                    fields,
                }))
                ...
                let type_name = ir.shapes[mapping.shape_id.0 as usize].name.clone().unwrap_or_else(|| "_".into());
                AsmValue::DefRef(AsmDefRef { type_name, name: mapping.name.clone() })
            }
        }
    ...
    match v {
        ...
        IrValue::Variant(tid, idx, payload) => {
            ...
            let variant_str = match &td.body {
                IrShapeBody::Enum(vs) => match vs.get(*idx as usize)
                    .and_then(|r| r.segments.first())
                    .map(|s| &s.value)
                {
                    Some(IrRefSegValue::Plain(p)) => p.clone(),
                    Some(IrRefSegValue::Shape(vtid)) =>
                        ir.shapes[vtid.0 as usize].name.clone().unwrap_or_else(|| "_".into()),
                    _ => "_".into(),
                },
                _ => "_".into(),
            };
...
fn ir_value_to_json_via(v: &IrValue, ir: &IrRes, cache: &HashMap<DefId, AsmDef>) -> serde_json::Value {
    match v {
        IrValue::Inst(fid) => {
