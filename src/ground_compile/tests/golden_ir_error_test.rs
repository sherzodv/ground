...
fn struct_anonymous_link_list_unresolved_type() {
    ...
    assert!(
        ...
        "error should name the unresolved type: {out}"
    );
}
...
fn error_unknown_inst_type() {
    ...
    assert!(
        out.contains("ghost"),
        "error should name the unknown type: {out}"
    );
}
...
fn error_field_from_different_type_rejected() {
    ...
    assert!(
        out.contains("'a'"),
        "error should name the unknown field: {out}"
    );
}
...
fn error_composed_shape_redefines_inherited_field() {
    ...
    assert!(
        out.contains("image"),
        "error should name the duplicate inherited field: {out}"
    );
    assert!(
        out.contains("already exists"),
        "error should explain the inherited collision: {out}"
    );
}
...
fn error_invalid_enum_variant() {
    ...
    assert!(
        out.contains("99"),
        "error should mention the bad variant: {out}"
    );
}
...
fn error_planned_def_cannot_be_referenced() {
    ...
    assert!(
        out.contains("api"),
        "error should mention the planned def name: {out}"
    );
}
...
fn error_typed_enum_instance_wrong_type() {
    ...
    assert!(out.contains("ERR:"), "expected error, got: {out}");
    assert!(
        out.contains("not a variant"),
        "expected not-a-variant error, got: {out}"
    );
}
...
fn typed_path_value_segment_count_mismatch_errors() {
    ...
    assert!(out.contains("ERR:"), "expected error, got: {out}");
    assert!(
        out.contains("typed path"),
        "error should mention typed path: {out}"
    );
}
...
fn error_typed_enum_struct_without_hint() {
    ...
    assert!(out.contains("ERR:"), "expected error, got: {out}");
    assert!(
        out.contains("shape hint required"),
        "expected hint-required error, got: {out}"
    );
}
...
fn inst_inline_struct_type_hint_mismatch() {
    ...
    assert!(
        ...
        "expected mismatch error, got: {out}"
    );
}
...
fn error_invalid_ipv4_and_ipv4net() {
    ...
    assert!(out.contains("ERR:"), "expected error, got: {out}");
    assert!(
        out.contains("expected ipv4net"),
        "expected ipv4net error, got: {out}"
    );
    assert!(
        out.contains("expected ipv4"),
        "expected ipv4 error, got: {out}"
    );
}
...
fn error_use_pack_not_found() {
    ...
    assert!(
        out.contains("nonexistent"),
        "error should name the missing pack: {out}"
    );
}
...
fn error_mapper_fn_not_in_scope() {
    ...
    assert!(out.contains("ERR:"), "expected error, got: {out}");
    assert!(
        out.contains("make_label"),
        "error should name the missing function: {out}"
    );
    assert!(
        out.contains("not in scope"),
