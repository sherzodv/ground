...
use crate::ast2::{self, *};
use crate::ir2::*;
use crate::ir2::ScopeKind as IrScopeKind;
...
fn pass1_mirror_scopes(parse_scopes: &[AstScope], ctx: &mut Ctx) {
    ...
    for ast_scope in parse_scopes.iter().skip(1) {
        ...
        let kind   = match ast_scope.kind {
            ast2::ScopeKind::Pack => IrScopeKind::Pack,
            ast2::ScopeKind::Type => IrScopeKind::Type,
        };
    ...
    for item in items {
        match &item.inner {
            ...
            AstStructItem::TypeDef(_) => {
                // Inline type-defs are hoisted to the type scope by parse2;
...
fn resolve_list_item(v: &AstValue, patterns: &[IrRef], ctx: &mut Ctx, scope: ScopeId, loc: &IrLoc) -> IrValue {
    ...
    let inst_name = &r.segments[0].inner.value;
